...
def gen_struct(decl, prefix):
    ...
    for field in decl['fields']:
        ...
        if is_prim_type(field_type):
            l(f"    {as_d_prim_type(field_type)} {field_name};")
...
def gen_helpers(inp):
    ...
    if inp['prefix'] in ['sg_', 'sdtx_', 'sshape_']:
        ...
        l('Range asRange(T)(T val) @trusted {')
        l('    import std.traits;')
        l('    static if (isPointer!T) {')
        l('       return Range(val, T.sizeof);')
        l('    } else static if (is(T == float[]) || is(T == double[])) {')
        l('       auto arr = val.dup;')
        l('       return Range(&arr[0], arr.length * arr[0].sizeof);')
        l('    } else static if (is(T == struct)) {')
        l('       Range r = {ptr: cast(const(void)*)&val, size: T.sizeof};')
        l('       return r;')
        l('    } else {')
        l('       static assert(0, "Cannot convert to range");')
        l('    }')
        l('}')
        l('')
    if inp["prefix"] == "sdtx_":
        l("struct Writer {")
        l("    void write(R)(R range) if (isInputRange!R && is(ElementType!R : const(ubyte))) {")
        l("        foreach (b; range) {")
        l("            putc(b);")
        l("        }")
        l("    }")
